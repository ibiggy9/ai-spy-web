...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    // Default behavior based on subscription
    if (hasSubscription) {
      router.replace('/');
    } else {
      router.replace('/subscribe');
    }
  }, [authIsLoaded, isSignedIn, isLoading, hasSubscription, router, searchParams]);
  ...
  return (
    <div className="min-h-screen bg-zinc-900 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-slate-400 text-sm">Please wait, this may take a few seconds...</p>
      </div>
    </div>
  );
} 