...
// Simple rate limiting for token requests
...
export async function POST(request) {
  try {
    ...
    if (!tokenRateLimit(`token-${ip}`, 10, 60000)) { // 10 requests per minute
    ...
    // This environment variable is only available server-side
    ...
    tokenResponse.headers.set('Referrer-Policy', 'strict-origin-when-cross-origin');
    tokenResponse.headers.set('Cache-Control', 'no-store, no-cache, must-revalidate');
