...
import { RiArrowLeftLine, RiIdCardLine, RiCalendarLine, RiDownloadLine, RiCloseLine, RiCheckLine, RiRestartLine } from 'react-icons/ri';
...
export default function BillingPage() {
  ...
  const fetchBillingData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(`Failed to fetch billing data: ${errorData.error || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setToast({
          message: responseData.error || responseData.details || 'Failed to cancel subscription',
          type: 'error'
        });
      }
    } catch (error) {
  ...
  const handleReactivateSubscription = async () => {
    if (!confirm('Are you sure you want to reactivate your subscription? This will continue your billing cycle.')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setToast({
          message: responseData.error || responseData.details || 'Failed to reactivate subscription',
          type: 'error'
        });
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-zinc-900 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="mb-4">You need to be signed in to view your billing information.</p>
          <a href="https://accounts.ai-spy.xyz/sign-in" className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg">
            Sign In
          </a>
        </div>
      </div>
          ...
          {hasActiveSubscription && activeSubscription ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {activeSubscription.items[0] && formatCurrency(
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">
                    {(() => {
                      ...
                      if (!activeSubscription.current_period_end || activeSubscription.current_period_end === 0) {
                        // Try to calculate from current_period_start + 1 month for monthly subscriptions
                        if (activeSubscription.current_period_start && activeSubscription.items[0]?.price.recurring.interval === 'month') {
                          const startDate = new Date(activeSubscription.current_period_start * 1000);
                          const calculatedEnd = new Date(startDate.getFullYear(), startDate.getMonth() + 1, startDate.getDate());
                      ...
                      const formattedDate = formatDate(activeSubscription.current_period_end);
                ...
                <div>
                  ...
                  <p className={`text-lg font-semibold flex items-center ${getStatusColor(activeSubscription.status)}`}>
                 ...
                 <div className="bg-orange-500/20 border border-orange-500/50 rounded-lg p-4">
                   <p className="text-orange-300 mb-3">
                     <strong>Subscription Cancellation Scheduled:</strong> Your subscription will end on {
                       (() => {
                         const formattedDate = formatDate(activeSubscription.current_period_end);
                         return formattedDate === 'Invalid Date' ? 'your current period end date' : formattedDate;
                       })()
                     }. You will continue to have access until then.
                   </p>
                   <p className="text-orange-200 text-sm">
                     Changed your mind? You can reactivate your subscription to continue billing as normal.
                   </p>
                 </div>
               )}
            ...
            <div className="text-center py-8">
              <div className="bg-slate-700/50 rounded-lg p-6 max-w-md mx-auto">
                ...
                <p className="text-slate-300 mb-4">You don&apos;t have an active subscription.</p>
                <Link 
                  href="/subscribe" 
        ...
        <div className="bg-slate-800 rounded-lg p-6">
          ...
          {billingData?.invoices.length ? (
                  ...
                  {billingData.invoices.map((invoice) => (
                    <tr key={invoice.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                      <td className="py-3 px-4">
                        <span className="font-mono text-sm">{invoice.number || invoice.id.substring(0, 8)}</span>
                      </td>
                      <td className="py-3 px-4">{formatDate(invoice.created)}</td>
                      <td className="py-3 px-4">
                        {(() => {
                          // Handle period display with better logic
                          if (!invoice.period_start && !invoice.period_end) {
                            return '-';
                          }
                          
                          const startDate = invoice.period_start ? formatDate(invoice.period_start) : null;
                          const endDate = invoice.period_end ? formatDate(invoice.period_end) : null;
                          
                          // If both dates are the same or end date is missing, try to calculate proper period
                          if (startDate && (!endDate || startDate === endDate)) {
                            // For monthly subscriptions, add one month to start date
                            if (invoice.period_start) {
                              const start = new Date(invoice.period_start * 1000);
                              const calculatedEnd = new Date(start.getFullYear(), start.getMonth() + 1, start.getDate());
                              return (
                                <span className="text-sm">
                                  {startDate} - {calculatedEnd.toLocaleDateString('en-US', {
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric',
                                  })}
                          ...
                          // Normal case with different start and end dates
                          if (startDate && endDate && startDate !== endDate) {
                            return (
                              <span className="text-sm">
                                {startDate} - {endDate}
                          ...
                          // Fallback to just start date if available
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-400 text-sm mt-2">Your invoices will appear here after your first payment.</p>
            </div>
          )}
        </div>
      </div>
