...
export default function Submission() {
    ...
    const [isPolling, setIsPolling] = useState(false);  // Explicit polling states
    const pollIntervalRef = useRef(null); // Ref to hold the interval ID
    const fileRef = useRef(null);  // Ref to hold the file object
    ...
    // Handler for when toast expires - do a final subscription check
    const handleToastExpire = async () => {
        try {
            ...
            if (finalResponse.hasSubscription && !subscriptionConfirmed) {
                ...
                // Show a brief "now active" message
                setTimeout(() => {
    ...
    useEffect(() => {
        ...
        if (urlParams.get('subscription') === 'success') {
            ...
            // Let the global subscription handler manage the polling
    ...
    const handleSubmit = async (file) => {
        ...
        try {
            ...
            const { signed_url, file_name, bucket } = await getUploadUrl(file.name, file.type);

            // 2. Upload to GCS immediately after getting URL (most critical part with 10s expiry)
            ...
            // 3. Start the report and transcription in parallel
            ...
            // Start polling with the task ID from the report
            setCurrentTaskId(reportResult.task_id);  // Store task ID for chat usage
    ...
    const handleChatSubmit = async (message) => {
        ...
        try {
            ...
            if (chatHistory.length === 0) {
                // First message: provide analysis data as initial context
            ...
            // Pass the subscription status and current task ID to sendChatMessage
            const data = await sendChatMessage(message, contextToSend, hasSubscription, currentTaskId);
            
            // Update conversation context with the response from backend
            if (data.context) {
                setConversationContext(data.context);
            }
            
            setChatHistory(prevChat => [...prevChat, {role: "assistant", content: data.response}]);
        } catch (error) {
    ...
    const handleReset = () => {
        ...
        setError(null);
        setChatHistory([]);
        setCurrentTaskId(null);  // Reset task ID when starting fresh
        setConversationContext(null);  // Reset conversation context when starting fresh
    };
    ...
    return (
        <div className="min-h-screen bg-zinc-900 fixed inset-0 flex">
            <div className={`${isMobile ? 'fixed bottom-0 w-full h-16' : 'fixed left-0 h-screen'} z-50`}>
                <Menu 
                    ...
                    >
                        <Accordion>
                            <AccordionItem 
                                ...
                                title={
                                    <span>
                                        AI-SPY <span className="bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-transparent bg-clip-text">STAC-2.2</span>
                                    </span>
                                }
                                subtitle={<span className="text-slate-300">Spacio-Temporal Audio Classifier</span>}
                            >
                                STAC-2.2 delivers efficient and precise AI detection through advanced spatio-temporal audio analysis, optimized for real-world performance.
                            </AccordionItem>
                        </Accordion>
                    </motion.div>
                        ...
                        {isAnalyzing ? (
                            isMobile ? (
                                ...
                                >
                                    ...
                                    <div className="text-slate-200 text-lg text-center">Processing File...</div>
                                    <div className="text-slate-500 text-sm text-center mt-2">Please wait while we analyze your file</div>
                                </motion.div>
                            ) : (
                                <motion.div 
                                    initial={{ scale: 0.9, opacity: 0 }}
                                ...
                                >
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 md:gap-4">
                                            {[...Array(3)].map((_, i) => (
                                                <Skeleton key={i} className="rounded-lg bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
                                        ...
                                        <div className="text-slate-200 text-lg md:text-xl text-center">Processing File...</div>
                                        <div className="text-slate-500 text-xs md:text-sm text-center">Please wait while we analyze your file</div>
                                    </div>
                                </motion.div>
                            )
                        ) : analysisResult ? (
            ...
            <Toast
                ...
                title={subscriptionConfirmed ? "Welcome to AI-SPY Pro!" : "Payment Successful!"}
                message={subscriptionConfirmed ? "Your Pro subscription is now active! Enjoy unlimited AI detection." : "Activating your Pro subscription... This may take up to 2 minutes while we process your payment."}
                type="success"
