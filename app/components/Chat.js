...
export default function Chat({ chatHistory, onChatSubmit, isChatLoading, analysisResult, transcriptionData }) {
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 w-full mb-6">
                  <div
                    ...
                    onClick={() => handlePresetClick("Explain the results to me.")}
                  >
                    <div className="text-blue-400 text-xs font-medium mb-1 group-hover:text-blue-300">Quick start</div>
                    Explain the results to me.
                  </div>
                  <div
                    className="p-4 rounded-2xl bg-gradient-to-br from-slate-700/80 to-slate-800/80 border border-slate-600/50 text-slate-200 text-sm cursor-pointer hover:from-slate-600/80 hover:to-slate-700/80 transition-all duration-200 backdrop-blur-sm group"
                    onClick={() => handlePresetClick("How did the model get to these results?")}
                  >
                    <div className="text-purple-400 text-xs font-medium mb-1 group-hover:text-purple-300">Deep dive</div>
                    How did the model get to these results?
                  </div>
                  <div
                    className="p-4 rounded-2xl bg-gradient-to-br from-slate-700/80 to-slate-800/80 border border-slate-600/50 text-slate-200 text-sm cursor-pointer hover:from-slate-600/80 hover:to-slate-700/80 transition-all duration-200 backdrop-blur-sm group"
                    onClick={() => handlePresetClick("What should I know about this file?")}
                  >
                    <div className="text-green-400 text-xs font-medium mb-1 group-hover:text-green-300">Key insights</div>
          ...
          <div className="space-y-4">
            <AnimatePresence>
              {chatHistory.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-[80%] p-4 rounded-2xl break-words ${msg.role === 'user'
                ...
                >
                  <div className="max-w-[80%] p-4 rounded-2xl py-6 bg-gradient-to-br from-slate-700/80 to-slate-800/80 border border-slate-600/50 backdrop-blur-sm">
                    <div className="flex space-x-2 items-center">
                      ...
                      <span className="text-slate-300 text-sm ml-2">Ai-SPY is thinking...</span>
                    </div>
                  </div>
                </motion.div>
              )}
        ...
        <div className="relative">
          <div className="flex items-center bg-gradient-to-r from-zinc-800/90 to-slate-800/90 rounded-2xl border border-zinc-600/50 backdrop-blur-sm overflow-hidden shadow-lg">
            ...
            <Tooltip content="Send message">
              ...
              >
                <IoSend size={20} className="group-hover:scale-110 transition-transform duration-200" />
              </button>
            </Tooltip>
          </div>
          {message.trim() && (
