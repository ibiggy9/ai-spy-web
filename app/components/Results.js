...
//import TimelineChart from './TimelineChart'; // Removed import
import TimelineGrid from './TimelineGrid';
import { useEffect, useState } from 'react';
import { useSubscription } from '../contexts/SubscriptionContext';
import { useAuth } from "@clerk/nextjs";

export default function Results({ result, onReset, chatHistory, onChatSubmit, isChatLoading, transcriptionData }) {
  ...
  const clips = Array.isArray(result?.result)
    ? result.result.filter(item => item.timestamp !== undefined)
        .map(item => ({
  ...
  const prepareChartData = (clips) => {
      return clips.map(clip => ({
          ...
          confidence: parseFloat(clip.confidence) * 100, // Keep as percentage
  ...
  const chartData = prepareChartData(clips);
    ...
    if (overallPrediction === 'AI') {
        shadowStyle = { boxShadow: '0 0 15px 5px rgba(255, 0, 0, 0.7)' }; // Red glow
    } else if (overallPrediction === 'Human') {
        shadowStyle = { boxShadow: '0 0 15px 5px rgba(0, 255, 0, 0.7)' }; // Green glow
    } else if (overallPrediction === 'Uncertain') {
        shadowStyle = { boxShadow: '0 0 15px 5px rgba(255, 255, 0, 0.7)' }; // Yellow glow
    } else if (overallPrediction === 'Mixed') { // Add Mixed shadow
        shadowStyle = { boxShadow: '0 0 15px 5px rgba(168, 85, 247, 0.7)' }; // Purple glow
    }
    ...
    return (
      <div className="bg-slate-800/50 border border-slate-600 rounded-lg p-4 mb-6">
        <div className="flex items-center justify-center">
          ...
          <span className="text-slate-300 text-sm">Checking subscription status...</span>
        </div>
      </div>
    );
  };
  ...
  const renderUpgradePrompt = () => {
    ...
    return (
      <div className="bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 border border-purple-500 rounded-lg p-4 mb-6">
        <h3 className="text-base text-white font-medium mb-2">Limited Results</h3>
        ...
        <ul className="text-sm text-gray-300 list-disc pl-5 mb-3">
          ...
          <li>AI chat assistance</li>
                ...
                <a
          href={isSignedIn ? "/subscribe" : "https://accounts.ai-spy.xyz/sign-in"}
  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-800 border border-zinc-700 rounded-lg p-3 md:p-3">
        ...
        <TimelineGrid chartData={chartData} formatXAxis={formatXAxis} transcriptionData={combinedTranscriptionData} />
      ...
      {!isLoading && !hasSubscription ? (
        <div className="bg-slate-800 border border-zinc-700 rounded-lg p-4 mt-6">
          <h3 className="text-base text-white font-medium mb-2">AI Chat Assistant</h3>
          <p className="text-sm text-gray-300 mb-3">
            Chat with our AI to analyze your results, get insights, and understand potential issues.
          </p>
          <a 
            href={isSignedIn ? "/subscribe" : "https://accounts.ai-spy.xyz/sign-in"}
            className="inline-block bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white px-4 py-2 rounded-md text-sm font-medium hover:opacity-90 transition-opacity"
