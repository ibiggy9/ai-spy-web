...
export default function SubscribeButton() {
  ...
  // Note: Subscription success handling is now managed globally in SubscriptionContext

  const handleSubscribe = async () => {
    try {
      // If not logged in, redirect to external sign in
      // Clerk's force redirect URLs will handle bringing them back to /subscribe
      if (!isSignedIn) {
      ...
      if (!errorMessage) {
        setErrorMessage("Sorry, there was a problem starting your subscription. Please try again.");
        setShowErrorModal(true);
      }
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Button
        ...
        size="lg"
      >
        {loading ? "Processing..." : isSignedIn ? "Start Unlimited Checks" : "Sign In to Continue"}
      </Button>
