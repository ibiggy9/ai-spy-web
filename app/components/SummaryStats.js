...
export default function SummaryStats({ stats }) {
            ...
            <div className="flex flex-col gap-3 mb-6">
                <Card className="bg-slate-800">
                    <CardBody className="p-3">
                        <div className="text-gray-400 text-center">No summary statistics available.</div>
                    </CardBody>
                </Card>
            </div>
        );
    ...
    const overallConfidence = stats?.aggregate_confidence !== undefined ? (stats.aggregate_confidence * 100).toFixed(2) : 'N/A';
    const overallPrediction = stats?.overall_prediction;

    // Calculate uncertain percentage (used for non-mixed cases)
    ...
    switch (overallPrediction) {
        case "AI":
            ...
            overallAssessment = overallConfidence >= 70
        ...
        case "Mixed":
            circleLabel = "Mixed";
            classificationClassName += "bg-purple-700/20 text-purple-700"; // Purple
            overallAssessment = overallConfidence >= 70
        ...
        case "Uncertain":
            ...
            break;
        default:
            circleLabel = "?";  // Fallback - Should not happen
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative w-[200px] h-[200px]">
                            <svg width="100%" height="100%" viewBox="0 0 100 100">
                                ...
                                {/* Conditional Rendering for Mixed vs. Other Cases */}
                                    ...
                                    <>
                                      {/* AI Circle */}
                                        ...
                                        <circle
                                            ...
                                            strokeDashoffset={-(circumference / 4 + (aiPercentage / 100) * circumference)}
                                            stroke="#facc15"
                                            strokeLinecap="round"
                                        />
                                        ...
                                        <circle
                                            ...
                                            strokeDashoffset={-(circumference / 4 + (aiPercentage / 100) * circumference + (uncertainPercentage / 100) * circumference)}
                                            stroke="#22c55e"
                                            strokeLinecap="round"
                                        />
                                    </>
                            ...
                            {/* Center Label - No "?" for valid predictions */}
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                                <span className="text-4xl font-semibold text-slate-200">{circleLabel}</span>
                            </div>
                        </div>

                        <div className="flex-1">
                            <p className="text-base md:text-lg text-white">
                                {overallAssessment} <span className={classificationClassName}>{overallPrediction}</span>
                            </p>
