...
export default function Submission_box({ onSubmit, isAnalyzing, error }) {
  ...
  const isValidFileType = (file) => {
    ...
    const extensionValid = fileName.endsWith('.mp3') || fileName.endsWith('.wav');
  ...
  // Simplify handleSubmit to just call onSubmit
  ...
  return (
    ...
    >
      ...
      <div className=' px-2 pb-5 text-slate-500 text-sm '>Drop or upload a file to get started.</div>
      ...
      >
        {!selectedFile ? (
          <div className='flex justify-center flex-col items-center'>
            <div className='text-white text-md mt-3'>Drag and drop an audio file to upload...</div>
            <div className='text-slate-500 text-xs my-3 w-60 text-center'>Accepts: Mp3 and WAV files only (Max 40MB)</div>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0.5, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, type: 'tween' }} className='flex flex-col items-center'>
        ...
        <div className='flex justify-center items-center w-60'>
          ...
          <div className='text-white flex flex-row' onClick={() => fileInputRef.current.click()}>
            <button className='underline'>{selectedFile ? "Choose different file" : "Click to upload "}</button>
