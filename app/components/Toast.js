...
export default function Toast({ 
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      if (remaining <= 0) {
        onExpire?.(); // Call onExpire first for any cleanup/checks
        onClose?.();
      }
    };
  ...
  const handleMouseEnter = () => {
    if (!isVisible) return;
    setIsPaused(true);
    // Save the remaining time when we pause
    const elapsed = Date.now() - startTimeRef.current;
  ...
  const handleMouseLeave = () => {
    if (!isVisible) return;
    setIsPaused(false);
    // Reset the start time for the remaining duration
    startTimeRef.current = Date.now();
  };
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className={`${config.bgColor} ${config.borderColor} border backdrop-blur-sm rounded-lg p-4 shadow-2xl relative overflow-hidden`}>
            {/* Progress bar */}
            {showProgressBar && (
