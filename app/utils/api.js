...
export async function fetchWithAuth(endpoint, options = {}) {
  try {
    ...
    if (response.status === 401) {
      ...
      if (!(options.body instanceof FormData)) {
        refreshHeaders['Content-Type'] = refreshHeaders['Content-Type'] || 'application/json';
      }
...
export async function getReportStatus(taskId, hasSubscription = false) {
  const response = await fetchWithAuth(`/report-status/${taskId}?has_subscription=${hasSubscription}`);
...
export async function transcribeAudio(file, hasSubscription = false) {
  ...
  const response = await fetchWithAuth(`/transcribe?has_subscription=${Boolean(hasSubscription)}`, {
    method: 'POST',
    body: formData
  });
...
export async function sendChatMessage(message, context, hasSubscription = false, taskId = null) {
