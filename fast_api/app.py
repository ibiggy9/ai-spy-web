from fastapi import FastAPI, UploadFile, HTTPException, Request, Depends, Security, status, Header, File
...
try:
    ...
    logger.warning("requests library not available, fallback transcription method will not work")
...
if not deepgram_api_key:
    logger.warning("No Deepgram API key found! Transcription will not work.")
...
SECURITY_CONFIG = {
    ...
    "cors": {
        "allowed_origins": os.getenv('ALLOWED_ORIGINS', 'https://app.ai-spy.xyz').split(','),
...
# Server-side subscription validation function (FIXED)
async def validate_subscription_claim(user_id: str, claimed_subscription: bool) -> bool:
    """
    PRODUCTION NOTE: This function currently trusts frontend subscription status.
    
    For enhanced security, integrate with Stripe API:
    
    Example implementation:
    try:
        customers = stripe.Customer.list(metadata={'userId': user_id}, limit=1)
        if customers.data:
        ...
        return claimed_subscription  # Fallback to frontend validation
    
    Current implementation trusts frontend validation due to architectural constraints.
    Frontend has proper access to Clerk user data and performs accurate Stripe checks.
    """
    try:
        # Log subscription claims for security monitoring
        await log_security_event(
            event_type="subscription_claim",
            user_id=user_id,
            details={
                "claimed_subscription": claimed_subscription,
                "note": "Trusting frontend validation - consider Stripe API integration for enhanced security"
            }
        )
        
        # Trust the frontend's subscription status
        # Frontend has access to real Clerk user IDs and performs proper Stripe validation
        return claimed_subscription
                
    except Exception as e:
        # Log subscription validation errors
        await log_security_event(
            event_type="subscription_validation_error",
            user_id=user_id,
            details={"error": str(e)}
        )
        # On error, return the claimed status (don't punish users for system errors)
        return claimed_subscription
...
async def transcribe_audio(request: Request, file: UploadFile, has_subscription: bool = False, authorization: str = Header(None)):
    # Get user ID for audit log
    token = authorization.replace("Bearer ", "")
    is_valid, user_id = validate_auth_token(token)
    
    # Validate subscription claim server-side
    validated_subscription = await validate_subscription_claim(user_id, has_subscription)
...
async def get_report_status(task_id: str, has_subscription: bool = False, authorization: str = Header(None)):
    # Get user ID for audit log
    token = authorization.replace("Bearer ", "")
    is_valid, user_id = validate_auth_token(token)
    ...
    print(f"Job contains file_name: {job.get('file_name', 'NOT SET')}")

    if "status" not in job:
        return {"status": "error", "error": "Invalid job structure", "results": None}
    
    # For free users, limit timeline data to only first 3 timestamps
    if not validated_subscription and job["status"] == "completed" and "result" in job:
        ...
        # Extract just the first 3 timeline data points (after the summary stats)
        timeline_items = [item for item in job["result"] if "timestamp" in item][:3]
        
        # Create a new limited result array with just the summary and 3 timestamps
        limited_result = []
        if summary_stats:
            limited_result.append(summary_stats)
        limited_result.extend(timeline_items)
        
        # Also limit transcription data if present
        ...
        return {
            ...
            "file_name": job.get("file_name"),  # Include file name for free users too
            "is_limited": True  # Flag to indicate limited data
        }
...
async def process_report(request: Request):
    ...
    try:
        ...
        try:
            ...
            if '-' in file_name:
                # Remove timestamp prefix like "1234567890.123-"
...
async def chat_with_gemini(request: Request, chat_request: ChatRequest, has_subscription: bool = False, task_id: str = None, authorization: str = Header(None)):
    ...
    await log_security_event(
        ...
        user_id=user_id,
        details={
            "claimed_subscription": has_subscription,
...
# Security status endpoint removed for production security

# @app.get("/security-status")
# async def security_status():
#     return {"status": "Security endpoint disabled in production"}
...
async def transcribe_audio_file(file_path):
    ...
    try:
        ...
        if hasattr(response, 'results'):
            ...
            # Direct access to response for newer API versions
            result = response
